...
def classifier_accuracy(scores: Tensor, true_classes: Tensor) -> Tensor:
    ...
    Use t.argmax.
    """
    return (scores.argmax(dim = 1) == true_classes).sum(dim = -1) / scores.shape[0]
...
def batched_softmax(matrix: Tensor) -> Tensor:
    ...
    """
    log_denom = batched_logsumexp(matrix)

    log_denom = einops.rearrange(log_denom, "b -> b 1")

    return t.exp(matrix - log_denom)
...
def batched_cross_entropy_loss(logits: Tensor, true_labels: Tensor) -> Tensor:
    ...
    lprobs = batched_logsoftmax(logits)
    indexes = einops.rearrange(true_labels, "n -> n 1")
    pred_at_index = lprobs.gather(1, indexes)
